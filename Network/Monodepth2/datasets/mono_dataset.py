...
class MonoDataset(data.Dataset):
    ...
    def preprocess(self, inputs, color_aug):
        ...
        """Create color_aug object and apply same augmentations to all images
        """
        # Resize color images
        # Create color_aug object by each scales
        for k in list(inputs):
            f=inputs[k]
            if 'color' in k:
                n, im, i =k
                # n : "color", im: frame_id (0, -1, 1), i: scale (0, 1, 2, 3)
                for i in range(self.num_scales): 
                    # 0, 1, 2, 3
                    # inputs[n, im, -1] 이 존재: Resize native Image
                    inputs[(n+"_aug", im, i)]=self.resize[i](inputs[n, im, i-1]) 
    ...
    def __getitem__(self, index):
        ...
        for i in self.frame_idxs:
            ...
            inputs[('color', i, -1)]=self.get_color(folder, frame_index+1, side, do_flip)
